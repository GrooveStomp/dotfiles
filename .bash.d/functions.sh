...
function bundle-install() {
    set -x
    bundle install --path $(pwd)/.env/gems --binstubs $(pwd)/.env/bin
    set +x
}
...
function app-grep() {
    grep "$@" | grep -v "^spec" | grep -v schema | grep -v "^coverage" | grep -v "^vendor" | grep -v "^log" | grep -v "^tmp"
}

function app-search() {
    ag "$@" | ag -v "^spec" | ag -v schema | ag -v "^coverage" | ag -v "^vendor" | ag -v "^log" | ag -v "^tmp" | ag -v "\.xml:"
}

function app-tree() {
    tree -I "env|spec|coverage|vendor|log|tmp" "$@"
}
...
function rmq-publish() {
    ...
    echo "rabbitmqadmin publish exchange=fanout routing_key=\"$routing_key\" properties=\"$properties\" payload=\"$payload\""
...
function display-configure() {
    ...
    function usage() {
        echo
        echo "Usage: ${FUNCNAME[0]} [options] mode"
    ...
    if [ ! -z "$configure_for_work" ]; then
        ...
        xrandr --output HDMI2 --mode 3840x2160 --dpi 160 # 4k work monitor.
    fi
}
